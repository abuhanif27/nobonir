...
from ai_engine.services.chat_assistant_service import generate_assistant_response
from django.contrib.auth import get_user_model

User = get_user_model()
user = User.objects.first() or User.objects.create_user(email="pro_test@example.com", password="password")

def test_scenario(name, message, auth=True):
...
# 1. Greeting
test_scenario("Greeting", "Hi there, how are you today?")

# 2. Help/Capabilities
test_scenario("Help Query", "What can you do for me?")

# 3. Product inquiry
test_scenario("Product Inquiry", "Do you have any air fryers in stock?")

# 4. Budget search
test_scenario("Budget Search", "I need a blender under 4000")

# 5. Order help (Guest)
test_scenario("Order Help (Guest)", "Where is my order?", auth=False)

# 6. Order help (Auth)
test_scenario("Order Help (Auth)", "Where is my order?", auth=True)
