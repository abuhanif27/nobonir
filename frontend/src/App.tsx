...
function App() {
  ...
  const loadCatalog = async () => {
    const [categoryRes, productRes] = await Promise.all([
      ...
      endpoints.products({ search, category, sort, in_stock: stockOnly || undefined }),
    ])
  ...
  const wishlistProductIds = useMemo(() => new Set(wishlistItems.map((item) => item.product.id)), [wishlistItems])
  const cartTotal = useMemo(
    () => cartItems.reduce((total, item) => total + item.product.discounted_price * item.quantity, 0),
    [cartItems]
  )

  const addToCart = async (productId: number) => {
    await endpoints.cart.add({ customer_id: customerId, product_id: productId, quantity: 1 })
  ...
  const toggleWishlist = async (productId: number) => {
    ...
    if (existing) {
      ...
      await endpoints.wishlist.add({ customer_id: customerId, product_id: productId })
    }
    await loadUserData(customerId)
  }
  ...
  const submitReview = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setReviewForm({ product_id: "", customer_name: "", rating: "5", comment: "" })
    await loadCatalog()
    await loadUserData(customerId)
  }
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="bg-primary py-8 text-primary-foreground">
        <div className="container">
          <h1 className="text-4xl font-bold tracking-wide">NOBONIR E-COMMERCE</h1>
          <p className="mt-2 text-lg font-semibold text-primary-foreground/90">SOFT STYLE SMART SHOPPING</p>
          <p className="mt-4 text-sm text-primary-foreground/90">Customer Session: {customerId}</p>
        </div>
      </header>
      ...
      <main className="container py-8">
        <div className="mb-6 grid gap-4 md:grid-cols-2">
          <Card className="border-none bg-secondary">
            <CardHeader>
              ...
              <CardDescription>
                Complete React + Django + AI commerce platform with product discovery, cart, wishlist, reviews,
          ...
          <Card className="border-none bg-secondary">
            <CardHeader>
              ...
              <CardDescription>
                Product module, inventory management, secure checkout, and AI recommendations integrated in one system.
              </CardDescription>
            </CardHeader>
          </Card>
        </div>
        ...
        <Card className="mb-6 border-none bg-card">
          <CardHeader>
            ...
            <CardDescription>Styled close to your reference with rounded cards and teal dashboard layout.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-3 md:grid-cols-4">
              ...
              <Button variant={stockOnly ? "default" : "outline"} onClick={() => setStockOnly((previous) => !previous)}>
            ...
            <Card key={product.id} className="bg-secondary/60">
              <CardHeader>
                <div className="flex items-start justify-between gap-2">
                  ...
                  {product.offer_percent > 0 && <Badge>{product.offer_percent}% OFF</Badge>}
                </div>
                <CardDescription>{product.category.name}</CardDescription>
              </CardHeader>
              <CardContent>
                <p className="mb-2 text-sm text-muted-foreground">{product.description}</p>
                <p className="font-semibold">৳{product.discounted_price.toFixed(2)}</p>
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" variant="outline" onClick={() => toggleWishlist(product.id)}>
                    <Heart className="mr-2 h-4 w-4" /> {wishlistProductIds.has(product.id) ? "Saved" : "Wishlist"}
                  </Button>
                </div>
              </CardContent>
            </Card>
        ...
        <section className="mb-8 grid gap-4 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Personalized products based on cart, wishlist, and rating behavior.</CardDescription>
            </CardHeader>
              ...
              {recommended.map((item) => (
                <div key={item.id} className="flex items-center justify-between rounded-lg border p-3">
                  ...
                  <Badge variant="secondary">
                    <Sparkles className="mr-1 h-3 w-3" /> ৳{item.discounted_price.toFixed(0)}
                  </Badge>
                </div>
              ))}
            </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Complete checkout module with stock-aware order creation and tracking.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {cartItems.map((item) => (
                  <div key={item.id} className="grid grid-cols-[1fr_110px_60px] items-center gap-2 rounded-lg border p-2">
                    <div>
                      <p className="text-sm font-medium">{item.product.name}</p>
                      <p className="text-xs text-muted-foreground">৳{item.product.discounted_price.toFixed(2)}</p>
                    </div>
                    <Input
                      type="number"
                      min={1}
              ...
              <p className="mt-4 text-sm font-semibold">Total: ৳{cartTotal.toFixed(2)}</p>
              <form className="mt-4 space-y-2" onSubmit={submitCheckout}>
                <Input
                  placeholder="Customer Name"
                  value={checkoutForm.customer_name}
                  onChange={(event) =>
                    setCheckoutForm((previous) => ({ ...previous, customer_name: event.target.value }))
                  }
                  required
                />
                <Input
                  ...
                  onChange={(event) =>
                    setCheckoutForm((previous) => ({ ...previous, customer_email: event.target.value }))
                  }
                  required
                />
                <Textarea
                  ...
                  onChange={(event) =>
                    setCheckoutForm((previous) => ({ ...previous, shipping_address: event.target.value }))
                  }
                  required
                />
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Submit product ratings and comments for trustworthy customer feedback.</CardDescription>
            </CardHeader>
            <CardContent>
              <form className="space-y-2" onSubmit={submitReview}>
                <Select
                  ...
                  onChange={(event) =>
                    setReviewForm((previous) => ({ ...previous, product_id: event.target.value }))
                  }
                  required
                >
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setReviewForm((previous) => ({ ...previous, customer_name: event.target.value }))
                  }
                  required
                />
                <Select
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Track complete order lifecycle for customer and admin monitoring.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-2">
              {orders.map((order) => (
